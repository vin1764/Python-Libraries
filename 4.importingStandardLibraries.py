#importing the random library
import random #this library chooses any random value out of the given data
courses=['Math','PDS','Art','Physics','Hindi']

chosen_course=random.choice(courses)
print(chosen_course)

#MATH LIBRARY
import math

rads=math.radians(180) #this function converts any given degree value to its corresponding value in radians
print(rads)

degree=math.degrees(rads/2) #converts any given radian value to its corresponding degree value
print(degree)

print(math.sin(math.radians(90))) #sin functions returns the sin value of the entered radian value

print('\n',dir(math),'\n')
#['__doc__', '__loader__', '__name__', '__package__', '__spec__', 'acos', 'acosh', 'asin', 'asinh',
#  'atan', 'atan2', 'atanh', 'cbrt', 'ceil', 'comb', 'copysign', 'cos', 'cosh', 'degrees', 'dist',
#  'e', 'erf', 'erfc', 'exp', 'exp2', 'expm1', 'fabs', 'factorial', 'floor', 'fmod', 'frexp', 'fsum',
#  'gamma', 'gcd', 'hypot', 'inf', 'isclose', 'isfinite', 'isinf', 'isnan', 'isqrt', 'lcm', 'ldexp', 
# 'lgamma', 'log', 'log10', 'log1p', 'log2', 'modf', 'nan', 'nextafter', 'perm', 'pi', 'pow', 'prod',
#  'radians', 'remainder', 'sin', 'sinh', 'sqrt', 'tan', 'tanh', 'tau', 'trunc', 'ulp']

#DATE TIME library
import datetime
date=datetime.date.today()
print(date)

#CALENDAR library
import calendar
print(calendar.isleap(2017))